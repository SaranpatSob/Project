...
while True:
    ...
    RawLog = {"Timestamp": int(CaptiveTimestamp), "LoginDomain" : LoginDomain, "Username": LoginUsername, "IPv4": IPv4, "IPv6": IPv6, "Status": CaptiveStatus }
                ...
                if LoginLog[5] != "-":
                    if (IPaddress[0] == "158"):
                        if (int(int(IPaddress[2])/64) + 1 == Multiple_Login[LoginUsername]["Area"]):
                            ...
                            Alert = {"Alert_Type": "Login From Multiple Locations", "Detected_Time": int(Log_Timestamp), "Username": LoginUsername, "MACAddress": MAC_Address, "IPv4": IPv4, "IPv6": IPv6}
                            es.index(index='vstest1alertlog', body=Alert)
